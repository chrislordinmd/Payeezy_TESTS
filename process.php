...
class SoapClientHMAC extends SoapClient {
  public function __doRequest($request, $location, $action, $version, $one_way = NULL) {
	global $context;
	$hmackey = "rtQGfIgYsItRKkQJZyo_AHFVU8o3J6nB"; // <-- Insert your HMAC key here
	$keyid = "62348"; // <-- Insert the Key ID here
	$hashtime = date("c");
	$hashstr = "POST\ntext/xml; charset=utf-8\n" . sha1($request) . "\n" . $hashtime . "\n" . parse_url($location,PHP_URL_PATH);
	$authstr = base64_encode(hash_hmac("sha1",$hashstr,$hmackey,TRUE));
	if (version_compare(PHP_VERSION, '5.6.33') == -1) {
...
$trxnProperties = array(
  ...
  "CAVV"=>"",
  "Password"=>"hotdog10",					                //Gateway Password
  "CAVV_Algorithm"=>"",
